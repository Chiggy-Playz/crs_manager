...
class DriveHandler {
  ...
  Future<File?> fileExists(String name) async {
    ...
    if (results.data["files"].length > 0) {
      return File.fromMap(results.data["files"][0]);
    }

    return null;
  }

  Future<File> createFile(String name, String mimeType, String parent) async {
    String url = "https://www.googleapis.com/drive/v3/files";
    final data = {
      "name": name,
      "mimeType": mimeType,
    ...
    return File.fromMap(result.data);
  }

  Future<File> uploadFile(String fileId, Object data) async {
    String url =

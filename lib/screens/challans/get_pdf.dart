...
Future<String> makePdf(Challan challan) async {
  var pdf = await preparePdf(challan);
  ...
  if (Platform.isWindows) {
    ...
    challansDirectory = Directory('storage/emulated/0/CrsManager/Challans');
  }
...
Future<PdfDocument> preparePdf(Challan challan) async {
  var pdf = PdfDocument();
  ...
  page.graphics.drawString(
      "Challan No. : ${challan.number.toString()} / ${challan.session}",
      normalFont,
  ...
  grid.columns[0].format = PdfStringFormat(alignment: PdfTextAlignment.center);
...
3. You are not allowed to break the seal. Also you are not allowed to open the machine.
  ...
  if (challan.cancelled) {
    final Uint8List imageData = File('Assets/cancelled.png').readAsBytesSync();
